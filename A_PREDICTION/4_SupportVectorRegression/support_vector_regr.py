...
sclx = StandardScaler()
scaled_x = sclx.fit_transform(X)

scly = StandardScaler()
scaled_y = scly.fit_transform(Y)

#%%
# SVR modeli oluşturun.
from sklearn.svm import SVR

svr_model = SVR(kernel="rbf")
svr_model.fit(scaled_x, scaled_y)

plt.scatter(scaled_x, scaled_y)
plt.plot(scaled_x, svr_model.predict(scaled_x), color="green")
plt.show()
...
print(r2_score(scaled_y, svr_model.predict(scaled_x)))
